...
class Mp4Segmenter extends Transform {
    ...
    get initSegment() {
        if (this._initSegment) {
            ...
            return null;
            //console.log('init segment not created yet');
        }
    }
    ...
    set initSegment(value) {
        ...
        if (index === -1) {
            console.log('header does not contain codec information');
        }
        ...
        this.emitter.emit('initSegment', this._initSegment)
    ...
    _findFtyp(chunk) {
        ...
        if (this._ftypLength < chunk.length) {
            ...
            console.log('ftypLength greater than chunkLength');
    ...
    _findMoov(chunk) {
        ...
        if (moovLength < chunkLength) {
            ...
            console.log('moovLength greater than chunkLength');
            this.emitter.emit('exit', {code: 1});
    ...
    _findMoof(chunk) {
        ...
        if (chunk[4] !== 0x6D || chunk[5] !== 0x6F || chunk[6] !== 0x6F || chunk[7] !== 0x66) {
            ...
            if (this._foundSegment === false) {
                ...
                console.log('failed to find moof after already running good');
                this.emitter.emit('exit', {code: 1});
